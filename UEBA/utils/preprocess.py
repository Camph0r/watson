...
def detect_high_resource_usage(df, hostname):
    z_scores = (
        df[["cpu_percent", "mem_percent", "threads"]]
        - df[["cpu_percent", "mem_percent", "threads"]].mean()
    ) / df[["cpu_percent", "mem_percent", "threads"]].std()
...
def detect_anomalous_start_times(df, hostname):
    start_times = df.groupby("name")["created"].apply(
        lambda x: x.dt.hour.value_counts().idxmax()
    )
    result = df[
        df.apply(
            lambda row: row["created"].hour
            != start_times.get(row["name"], row["created"].hour),
            axis=1,
        )
    ]
    for _, row in result.iterrows():
