...
func (g *CertGenerator) GenerateCertificate(deviceID string, validFor int, domains []string, ipaddr []string) (certPEM, keyPEM []byte, fingerprint string, err error) {
	...
	os.WriteFile("client.crt", certPEM, os.FileMode(os.O_RDWR))
	os.WriteFile("client.key", keyPEM, os.FileMode(os.O_RDWR))
	return certPEM, keyPEM, fingerprint, nil
}

// func main() {
// 	g, _ := NewCertGenerator("ca.crt", "ca.key")
// 	_, _, f, _ := g.GenerateCertificate("test-client", 1000, []string{"localhost", "client"}, []string{"192.168.1.1", "127.0.0.1"})
// 	log.Println(f)
// }
